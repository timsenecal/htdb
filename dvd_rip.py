...
if parms == "read":
	...
	for line in lines:
		device_parts = string.split(device_dump)
		...
		#			print disc_title, title_num, title_time, title_chapters
					query = "insert into dvd_data (dvd_title, titlenum, titlelen, titlechaps) values ('"+disc_title+"', '"+title_num+"', '"+title_time+"', '"+title_chapters+"');"
					print query
					result = htdb_db.query(query)
...
if parms == "rip":
	...
	for line in lines:
		...
		device_parts = string.split(device_name)
		device_name = device_parts[0]
		
		device_path = string.strip(device_path)
		print device_name
		print device_path
...
#		sys.exit(0)
		...
		query = "select distinct titlenum, titlelen from dvd_data where dvd_title = '"+disc_title+"' and title_rip = 'yes';"
		print query
		result = htdb_db.query(query)
		result_list = list(result.dictresult())
...
#		sys.exit(0)
		...
		#	sys.exit(0)
			...
			query = "select distinct dvd_title, titlenum, titlelen from dvd_data where title_rip != 'delete' order by dvd_title, titlenum;"
			print query
			result = htdb_db.query(query)
			result_list = list(result.dictresult())
			...
			for record in result_list:
				...
				query = "update dvd_data set title_rip = 'yes' where titlenum = "+titlenum+";"
				result = htdb_db.query(query)
				
				handbrake_command = "/usr/bin/HandBrakeCLI --no-dvdnav -i '"+device_path+"' -t "+titlenum+" -f av_mp4 -O -e x264 --vfr -E ca_aac --gain 5.0 -o '"+tempdir+dest_title+".mp4'"
...
#			sys.exit(0)
			...
			query = "select distinct dvd_title, titlenum, titlelen from dvd_data where (title_rip = 'yes' or title_rip = 'nada');"
			print query
			result = htdb_db.query(query)
			result_list = list(result.dictresult())
			...
			for record in result_list:
				...
				handbrake_command = "/usr/bin/HandBrakeCLI --no-dvdnav -i '"+device_path+"' -t "+titlenum+" -f av_mp4 -O -e x264 --vfr -E ca_aac --gain 5.0 -o '"+tempdir+dest_title+"'"
				if string.find(dest_title, "'") > 0:
					handbrake_command = "/usr/bin/HandBrakeCLI -i '"+device_path+"' -t "+titlenum+" -f av_mp4 -O -e x264 --vfr -E ca_aac --gain 5.0 -o \""+tempdir+dest_title+"\""
	...
	eject_command = "/usr/bin/eject "+device_name
	print eject_command
