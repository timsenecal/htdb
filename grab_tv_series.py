...
query = "select distinct tconst, primarytitle as filename from tv_files where data_collected = 'no' and tconst != 'nada' order by primarytitle;"
print query
		...
		if (status == 0):
			...
			if (status == 0):
				...
				if (status == 0):
					...
					query = "update tv_files set data_collected = 'yes' where tconst = '"+tconst+"';"
					print query
					result = htdb_db.query(query)
			else:
...
print "total number of shows found:", total_updated
