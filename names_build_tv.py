...
#	sys.exit(0)
	...
	if string.find(buffer2, "complete, awaiting") > 0:
		buf_parts = string.split(buffer2, "<span> complete, awaiting verification </span>")
...
#					print "name line", str(int(title_row))+"::"+title_line
				...
				credits_counter_str = str(int(credits_counter))
				query_movie = "insert into movie_credits(tconst, nconst, profession, role, creditsnum) values ('"+tconst+"', '"+nconst+"', '"+profession+"', '"+character+"', '"+credits_counter_str+"');"
				print query_movie
				try:
...
query = "select distinct tconst, primarytitle from tv_files where data_collected = 'no';"

if string.find(clean, "tt") == 0:
	query = "select distinct tconst, primarytitle from tv_files where tconst = '"+clean+"';"
	
if string.find(clean, "2019") == 0:
	query = "select distinct tconst, primarytitle from tv_files where stamp > '"+clean+"';"
if string.find(clean, "202") == 0:
	query = "select distinct tconst, primarytitle from tv_files where stamp > '"+clean+"';"
